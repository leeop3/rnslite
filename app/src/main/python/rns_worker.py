...
class BTInterface(RNS.Interfaces.Interface.Interface):
    def __init__(self, owner, name, kt_service):
        ...
        RNS.log(f"BTInterface {name} synchronized with Reticulum core.")
...
lxm_router = None
inbox = []
# known_nodes: { "hash_hex": "Display Name" }
known_nodes = {}
...
def message_received(lxm):
    ...
    inbox.append({"sender": sender_hash, "content": content, "time": time.strftime("%H:%M")})

def announce_handler(aspect_filter, data, packet):
    # This is the "Sideband" discovery logic
    node_hash = RNS.prettyhexrep(packet.destination_hash)
    try:
        name = data.decode("utf-8")
        known_nodes[node_hash] = name
        RNS.log(f"Discovered: {name} ({node_hash})")
    except:
...
def start(storage_path, kt_service, display_name):
    ...
    # ANNOUNCE: This allows others to find us and get our public key
...
def send_lxm(dest_hex, text):
    try:
        ...
        # LXMF will look up the destination hash in its announce-table
        # and use the stored Public Key for encryption.
        ...
        return "Encryption & Queuing Success"
    except Exception as e: return str(e)

def get_updates():
    global inbox
    # Return nodes as list of strings "Name (Hash)"
    nodes_list = [f"{v} ({k})" for k, v in known_nodes.items()]
    data = {"inbox": list(inbox), "nodes": nodes_list}
    inbox = []
    return data