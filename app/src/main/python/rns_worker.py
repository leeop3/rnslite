...
def _encode_sideband_name(name):
    # Sideband msgpack format: [name, None]
    name_bytes = name.encode("utf-8")
    return bytes([0x92, 0xA0 + len(name_bytes)]) + name_bytes + bytes([0xC0])

def _decode_lxmf_name(data):
    if not data: return "Unknown"
    try:
        # Improved name extraction: find the first stretch of printable chars
        import re
        match = re.search(b"[\\x20-\\x7E]{2,}", data)
        return match.group(0).decode("utf-8") if match else "Mesh Node"
...
def start(storage_path, kt_service, display_name):
    ...
    # 1. LOCK IDENTITY BEFORE START (Sideband Method)
    ...
    # Force Reticulum to use this as its Primary identity
    storage_id_path = os.path.join(rns_dir, "storage", "identity")
    identity.to_file(storage_id_path)
    ...
    # 5. ANNOUNCE
    time.sleep(1.0)
    destination.announce(app_data=_encode_sideband_name(display_name))
